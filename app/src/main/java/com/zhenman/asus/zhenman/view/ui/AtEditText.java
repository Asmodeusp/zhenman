...
public class AtEditText extends EditText {
    ...
    public void addAtContent(String id, String content) {
        atList.add(new Entity(id, content));
        ...
        if (spanIndexes != null && spanIndexes.size() != 0) {
            for (int i = 0; i < spanIndexes.size(); i++) {
                if (i % 2 == 0) {  // 开始位置
                    ...
                    spanText.setSpan(new ForegroundColorSpan(context.getResources().getColor(R.color.c1)), startIndex, spanIndexes.get(i), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
                }
            }
        }
        return spanText;
